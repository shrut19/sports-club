...
public class Database {
    ...
    public static void insert_user (User user) {
         ...
         c = DriverManager
            .getConnection("jdbc:postgresql://localhostjava Main:5432/testdb",
            "postgres", "4113");
	...
	if (tables.next()) {
  		System.out.println("table already exisits "); 
	}
	else {
	  // Table does not exist
	
         stmt = c.createStatement();
         String INSERT_USERS_SQL = "INSERT INTO USERS (ID,NAME,AGE,ADDRESS,equip) "
            + "VALUES (?, ?, ?, ?, ?);";
             try (Connection connection = DriverManager
            .getConnection("jdbc:postgresql://localhost:5432/testdb",
            "postgres", "4113");
            PreparedStatement preparedStatement = connection.prepareStatement(INSERT_USERS_SQL)) {
            preparedStatement.setInt(1, Integer.parseInt(user.id));
